...
<template>
	<SlideTransition animation="shelf-down">
		<div
			...
			v-if="
				activeConversation.pinnedMessage && !activeConversation.pinnedMessageHidden
			"
		>
			...
			<div class="flex">
				...
				<IconButton
					...
					:class="{
						'mr-3': store.user && activeConversation?.admins?.includes(store.user.id),
					}"
				>
				...
				<IconButton
					v-if="store.user && activeConversation?.admins?.includes(store.user.id)"
